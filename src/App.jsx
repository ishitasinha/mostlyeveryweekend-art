...
export default function App() {
  ...
  const handleImageLoad = (e) => {
    // scale the original size down to one third
    setPreviewSize({ width: Math.round(e.target.naturalWidth / 3), height: Math.round(e.target.naturalHeight / 3) });
  };
    ...
    <div className="min-h-screen bg-gradient-to-b from-pink-100 via-purple-100 to-blue-100 text-gray-800 antialiased">
      <header className="max-w-5xl mx-auto p-6 flex items-center justify-between">
        ...
        <nav className="flex items-center gap-4">
          <a href="#about" className="text-sm px-3 py-2 rounded-lg hover:bg-purple-200 hover:text-purple-900">
            About
          </a>
          <a href="#gallery" className="text-sm px-3 py-2 bg-purple-500 text-white rounded-lg shadow hover:bg-purple-600">
            Explore
          </a>
        </nav>
      </header>
      ...
      <main className="max-w-5xl mx-auto p-6">
        <section id="about" className="mb-16 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          {preview && (
            ...
            >
              <img src={preview} alt="Preview" onLoad={handleImageLoad} style={{ display: "block" }} />
            </div>
          )}
        </section>
        ...
        <section id="gallery">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
            {galleryImages.map((src, idx) => (
              ...
              >
                <img src={src} alt={`Art ${idx + 2}`} className="w-full h-56 object-cover" onError={(e) => (e.target.style.display = "none")} />
                <figcaption className="p-3 text-sm text-purple-600 flex items-center justify-between">
                  ...
                  <a href={src} download className="text-xs px-2 py-1 border border-purple-300 rounded-lg hover:bg-purple-100">
                    Download
                  </a>
                </figcaption>
              </motion.figure>
      ...
      <footer className="mt-16 py-8 border-t border-purple-200 bg-purple-50">
        <div className="max-w-5xl mx-auto p-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="text-sm text-purple-700">✨ © {new Date().getFullYear()} MostlyEveryWeekend — Made with creativity & pastels</div>
          <div className="flex items-center gap-4 text-sm text-purple-600">
            <a href="https://instagram.com/mostlyeveryweekend" target="_blank" rel="noreferrer">Instagram</a>
